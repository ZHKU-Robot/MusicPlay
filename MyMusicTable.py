    ...
    def mouseDoubleClickEvent(self, e) -> None:
        if self.lastRow!=-1:
            ...
            if self.songPath not in self.mainWindow.historyTable:
                ...
                self.mainWindow.musicHistoryList.addMedia(QMediaContent(QUrl.fromLocalFile(file)))
            if self.type=='play':
                self.mainWindow.musicMediaPlayer.setPlaylist(self.mainWindow.musicPlayList)
                self.mainWindow.musicPlayList.setCurrentIndex(self.lastRow)
                self.mainWindow.curList=self.mainWindow.musicPlayList
            elif self.type=='history':
                self.mainWindow.musicMediaPlayer.setPlaylist(self.mainWindow.musicHistoryList)
                self.mainWindow.musicHistoryList.setCurrentIndex(self.lastRow)
                self.mainWindow.curList=self.mainWindow.musicHistoryList
